...
class CylinderFrustumDemoScreen extends StatefulWidget {
  CylinderFrustumDemoScreen({Key? key}) : super(key: key);

  @override
  State<CylinderFrustumDemoScreen> createState() =>
      _CylinderFrustumDemoScreenState();
}

class _CylinderFrustumDemoScreenState extends State<CylinderFrustumDemoScreen> {
  ...
  double bottomRadius = 300;
  double topRadius = 300;
  @override
...
class CylinderFrustum extends StatelessWidget {
  CylinderFrustum(
      {Key? key,
    ...
    return Container(
      ...
      child: Stack(children: [
        ...List.generate(framesCount, (i) {
          double w = 300;
          double h = 300;
          double minRad = m.min(bottomRadius, topRadius);
          double radDiff = (bottomRadius - topRadius).abs();
          if (bottomRadius > topRadius) {
            w = minRad + radDiff * ((framesCount - i) / framesCount);
            h = minRad + radDiff * ((framesCount - i) / framesCount);
          } else {
            w = minRad + radDiff * ((i) / framesCount);
            h = minRad + radDiff * ((i) / framesCount);
          }
          double maxSize = m.max(bottomRadius, topRadius);
          // double left = 0;
          // double top = 0;
          double left = maxSize * 0.5 - w * 0.5;
          double top = maxSize * 0.5 - h * 0.5;
          double zTranslate = 0;
